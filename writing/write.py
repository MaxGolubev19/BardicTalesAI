class Write:
    ...
    async def write_message(self, role: str, content: str):
    ...
    async def write_feedback(self, role: str, content: str):
        head = f'#{role}\n#{self.user_id} {self.user_name}:\n'
        for i in range(0, len(content), self.MAX_MESSAGE_LENGTH - len(head)):
    ...
    async def move(self, role: str, content: str):
        await self.write_message(role, content)

    async def feedback(self, content: str):
